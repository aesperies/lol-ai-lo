...
def _fake_extraction(monkeypatch: pytest.MonkeyPatch, payload: dict[str, Any]) -> None:
    ...
    monkeypatch.setattr(
        llm,
        "complete_json",
        lambda prompt, schema, *, max_tokens=8192, system=None, gestora_id=None, **kwargs: dict(payload),
    )
...
class TestTabularExtraction:
    ...
    def test_tag_column_constrains_to_options(self, db, seed, monkeypatch):
        ...
        def fake(prompt, schema, *, max_tokens=8192, system=None, gestora_id=None, **kwargs):
