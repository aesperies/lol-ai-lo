"""Request-workflow domain helpers (state machine, document lookup, Exit A gates).

Shared by the API routers AND the background pipelines
(services/generation_pipeline.py), so they live in the service layer.

Pragmatic note: ``transition``/``require_status`` raise HTTPException directly.
Inside a request handler that is the natural 409; inside a background job the
job runner treats it like any other failure. Kept as-is (rather than a domain
exception + translation layer) deliberately — one behavior, less machinery.
"""
